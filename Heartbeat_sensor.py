...
thresh = 525 # used to find instant moment of heart beat, seeded amp = 100 # used to hold amplitude of pulse waveform, seeded firstBeat = True # used to seed rate array so we startup with reasonable BPM 
...
if Signal < thresh and N > (IBI/5.0)*3: # avoid dicrotic noise by waiting 3/5 of last IBI 
if Signal < T: # T is the trough 
T = Signal # keep track of lowest point in pulse wave 
if Signal > thresh and Signal > P: 
P = Signal 
# signal surges up in value every time there is a pulse 
if N > 250: # avoid high frequency noise if Signal > thresh and Pulse == False and N > (IBI/5.0)*3: 
...
rate[-1] = IBI # add the latest IBI to the rate array runningTotal = sum(rate) # add upp oldest IBI values 
...
amp = P - T # get amplitude of the pulse wave thresh = amp/2 + T # set thresh at 50% of the 
amplitude 
...
secondBeat = False # when we get the heartbeat back self.BPM = 0 
45
time.sleep(0.005) 
# Start getBPMLoop routine which saves the BPM in its variable def startAsyncBPM(self): 
